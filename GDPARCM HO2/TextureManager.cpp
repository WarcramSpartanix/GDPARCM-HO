...
TextureManager::TextureManager()
{
	...
	assetLoader = new AssetLoaderAction("");
	batchAssetLoader = new BatchAssetLoaderAction(0, 0, nullptr);
	threadPool = new ThreadPool("ThreadPool", 12);
...
void TextureManager::loadSingleStreamAsset(int index, IExecutionEvent* execEvent)
{
	...
	for (const auto& entry : std::filesystem::directory_iterator(STREAMING_PATH)) {
		if(index == fileNum)
		{
			...
			this->assetLoader->SetPath(path);
			this->assetLoader->setExecEvent(execEvent);
			
			this->threadPool->scheduleTask(this->assetLoader);
			break;
		}
...
void TextureManager::loadBatchAsset(int index, AssetLoaderAction* assetLoaderAction, IExecutionEvent* execEvent)
{
	int fileNum = 0;
	...
	for (const auto& entry : std::filesystem::directory_iterator(STREAMING_PATH)) {
		if (index == fileNum)
		{
			...
			assetLoaderAction->SetPath(path);
			assetLoaderAction->setExecEvent(execEvent);

			this->threadPool->scheduleTask(assetLoaderAction);
			break;
		}
...
void TextureManager::loadBatchAsset(int index, int batchSize, IExecutionEvent* execEvent)
{
	batchAssetLoader->SetValues(index, batchSize, execEvent);
	threadPool->scheduleTask(batchAssetLoader);
}
